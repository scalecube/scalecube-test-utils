 ...
 * order &mdash; for example, when writing <em>integration tests</em> or <em>functional tests</em>
 * where the sequence of the tests is important, especially in conjunction with {@link
 * TestInstance @TestInstance(Lifecycle.PER_CLASS)}.
 *
 * <p>To control the order in which test methods are executed, annotate your test class or test
 * interface with {@link TestMethodOrder @TestMethodOrder} and specify the desired {@link
 * MethodOrderer} implementation.
 *
 * @since 5.0
 * @see DisplayName
 * @see RepeatInfo
...
public @interface Repeat {
   ...
   *
   * <ul>
   *   <li>{@link #DISPLAY_NAME_PLACEHOLDER}
   *   <li>{@link #CURRENT_REPETITION_PLACEHOLDER}
   *   <li>{@link #TOTAL_REPETITIONS_PLACEHOLDER}
   * </ul>
   *
   * <p>Defaults to {@link #SHORT_DISPLAY_NAME}, resulting in names such as {@code "repetition 1 of
   * 2"}, {@code "repetition 2 of 2"}, etc.
   *
   * <p>Can be set to <code>{@link #LONG_DISPLAY_NAME}</code>, resulting in names such as {@code
   * "myRepeatedTest() :: repetition 1 of 2"}, {@code "myRepeatedTest() :: repetition 2 of 2"}, etc.
   *
   * <p>Alternatively, you can provide a custom display name, optionally using the aforementioned
   * placeholders.
   *
   * @return a custom display name; never blank or consisting solely of whitespace
   * @see #SHORT_DISPLAY_NAME
   * @see #LONG_DISPLAY_NAME
   * @see #DISPLAY_NAME_PLACEHOLDER
   * @see #CURRENT_REPETITION_PLACEHOLDER
